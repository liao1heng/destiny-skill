...
- 使用该 skill 前先确保 `rg` 可用；如果当前系统没有 `rg`，先全局安装 `ripgrep`，不要把它写进项目依赖。
- Windows 优先用 `winget install --id BurntSushi.ripgrep.MSVC -e --accept-source-agreements --accept-package-agreements` 安装；安装后用 `rg --version` 验证，再继续探测项目。

## 先检查

0. 工具前置：
- `Get-Command rg` 或 `command -v rg`
- 找不到 `rg` 时先全局安装 `ripgrep`
- `rg --version`
